    ...
    for (const measure of meiDoc.querySelectorAll("measure")) {
        ...
        for (const staff of measure.querySelectorAll("staff")) {
            ...
            for (const layer of staff.querySelectorAll("layer")) {
                ...
                // Everything that takes time, in the order it is written. A chord
                // times its notes, so its own notes are not counted again.
                for (const event of layer.querySelectorAll("note, chord, rest, space, mRest, mSpace")) {
                    if (event.localName === "note" && event.closest("chord")) continue;
